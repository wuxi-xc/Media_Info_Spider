...
class BilibiliClient(AbstactApiClient):
    ...
    async def report_video(self, aid: int, reason: str = None) -> Dict:
        """
        Bilibili web video report api
        :param aid: 稿件avid
        :param reason: 举报理由
        :return:
        """
        uri = "/x/web-interface/appeal/v2/submit"
        csrf = self.cookie_dict.get("bili_jct", "")

        if reason is None:
            reason = "侵犯版权，涉嫌传播盗版资源"
        post_data = {
            "aid": aid,
            "desc": reason,
            "block_author": 'false',
            "tid": 10018, # 稿件存在传播或售卖假货、盗版资源等问题
            "attach": "",
            "csrf": csrf
        }
    ...
    async def forensics_by_id(self, context_page : Page, aid: Union[int, None] = None, bvid: Union[str, None] = None) -> Union[str, None]:
        """
        Bilibli web video detail forensics, aid 和 bvid任选一个参数
        :param aid: 稿件avid
        :param bvid: 稿件bvid
        :return:
        """
        try:
            if not aid and not bvid:
                raise ValueError("请提供 aid 或 bvid 中的至少一个参数")
            url = f'https://www.bilibili.com/video/av{aid or bvid}'
            await context_page.goto(url)
            await context_page.wait_for_load_state("load")
            await asyncio.sleep(3)

            img_path = f"img_store/bili_fornesics_av{aid or bvid}.png"
            await context_page.screenshot(full_page=True, path=img_path, timeout=90000)
            await context_page.close()
            return img_path
        except TimeoutError as e:
            await context_page.close()
            utils.logger.error(f"[BilibiliClient.forensics_by_id] video {aid or bvid} TimeOutError: {e}")
            return None
