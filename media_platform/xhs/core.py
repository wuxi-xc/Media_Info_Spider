...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            # stealth.min.js是一个js脚本，用于防止网站检测到爬虫
            await self.browser_context.add_init_script(path="libs/stealth.min.js")

            # 添加 cookie 属性 webId 以避免网页上出现滑动验证码
    ...
    async def search(self) -> None:
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while page * xhs_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                note_details = await asyncio.gather(*detail_task_list)
        ...
        async with semaphore:
            try:
                if config.ENABLE_FORENSICS:
                    await self.xhs_client.forensics_by_id(note_id=note_id, context_page= await self.browser_context.new_page())
