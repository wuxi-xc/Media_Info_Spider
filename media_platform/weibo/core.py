...
class WeiboCrawler(AbstractCrawler):
        ...
        for keyword in search_keyword.split(","):
            ...
            while page * weibo_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                if config.ENABLE_FORENSICS:
                    ...
                    task_list = [
                        self.get_note_forensics(note_id=note_id, semaphore=semaphore)
    ...
    async def get_note_forensics(self, note_id: str, semaphore: asyncio.Semaphore) -> None:
        """
        get note forensics
        :param note_id:
        :param semaphore:
        ...
        async with semaphore:
            try:
                await self.wb_client.forensics_by_id(await self.browser_context.new_page(), note_id)
            except DataFetchError as ex:
                utils.logger.error(f"[WeiboCrawler.get_note_forensics] get note_id: {note_id} forensics error: {ex}")
