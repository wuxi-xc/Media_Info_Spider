...
class XiaoHongShuClient(AbstactApiClient):
    ...
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        """
        封装httpx的公共请求方法，对请求以及响应进行修改预处理（httpx支持同步和异步请求，也可以是使用aiohttp进行异步请求）
        Args:
            method: 请求方法
            url: 请求的URL
    ...
    async def forensics_by_id(self, note_id: str, context_page: Page) -> Union[str, None]:
        """
        进行笔记截图取证
        Args:
            note_id: 笔记ID
        ...
        """
        try:
            final_url = f'/explore/{note_id}'
            await context_page.goto(f"{self._domain}{final_url}")
            await asyncio.sleep(2)
            if "请通过验证" in await context_page.content():
                utils.logger.info("[XiaoHongShuLogin.check_login_state] 登录过程中出现验证码，正在自动验证")
                await utils.correct_angle(context_page)
                await asyncio.sleep(3)

            img_path = f"img_store/xhs_fornesics_{note_id}.png"
            ...
            await context_page.close()
            utils.logger.error(f"[XiaoHongShuClient.forensics_by_id] note {note_id} TimeoutError: {e}")
            return None
