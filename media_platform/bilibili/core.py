...
class BilibiliCrawler(AbstractCrawler):
        ...
        async with semaphore:
            try:
                if config.ENABLE_FORENSICS:
                    await self.bili_client.forensics_by_id(await self.browser_context.new_page(),aid=aid, bvid=bvid)
                return await self.bili_client.get_video_info(aid=aid, bvid=bvid)
            except DataFetchError as ex:
    ...
    def format_proxy_info(ip_proxy_info: IpInfoModel) -> Tuple[Optional[Dict], Optional[Dict]]:
        ...
        httpx_proxy = {
            f"{ip_proxy_info.protocol}": f"http://{ip_proxy_info.user}:{ip_proxy_info.password}@{ip_proxy_info.proxy}"
        }
